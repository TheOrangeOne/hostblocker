...
(define (main)
  ...
  (define hf (hf-parse in))
  (close-input-port in)

  (define dec-hf (decorate hf (reverse (modifiers))))

  ;(define out (open-output-file (hostsfile-out-path) #:exists 'replace))
  ;(hf-print hf (current-output-port))
  ...
  (cond
    ...
    ; (hf-print hf (current-output-port))]
    [else (hf-print hf (current-output-port))])
  (void))
  ...
  (command-line
   ...
   [("-l" "--list")
    ...
    (define list-sources-dec
      (λ (x)
        (hf-list-sources x (current-output-port) (hostsfile-path))))
